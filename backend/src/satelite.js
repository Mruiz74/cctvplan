...
// Google Static Maps (satélite)
async function googleSatelite({ direccion, lat, lng, metros }) {
  ...
  const url = `https://maps.googleapis.com/maps/api/staticmap?center=${lat},${lng}&zoom=${z}&size=640x640&scale=2&maptype=satellite&key=${GOOGLE_MAPS_KEY}`;
  const r = await fetch(url);
...
// Esri World Imagery por mosaico de tiles (sin API key)
async function esriSatelite({ direccion, lat, lng, metros }) {
...
async function satelite({ direccion, lat, lng, metros }) {
  ...
  if (GOOGLE_MAPS_KEY) return googleSatelite({ direccion, lat, lng, metros });
  return esriSatelite({ direccion, lat, lng, metros });
}
