...
app.post('/api/autodiseno', async (req, res) => {
  const { imagenDataUrl, brief, pxPerMeter, planoW, planoH, catalogo } = req.body || {};
  if (!catalogo || !Array.isArray(catalogo) || catalogo.length === 0) {
    return res.status(400).json({ error: 'Falta el catálogo de cámaras' });
  }
  try {
    const diseno = await autoDiseno({ imagenDataUrl, brief, pxPerMeter, planoW, planoH, catalogo });
    res.json(diseno);
  } catch (e) {
